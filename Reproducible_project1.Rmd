...
```{r}
   g<-ggplot(averageSteps, aes(interval, steps))
```

  # Setting shape and color for the plot
```{r}
   g<-g+geom_line(col="blue", lwd=1.2)

  # Setting titles for the plot

 g<-g+ggtitle("Average daily activity pattern")+ xlab ("Time Interval") + ylab("Steps")
...
g<-ggplot(ImputedSteps, aes(Steps))

# setting shape and colors for the plot

g<-g+geom_histogram(boundary=0, binwidth=2000, col="chartreuse4", fill="chartreuse3")

# setting titles for the plot

g<-g+ggtitle("Histogram of the steps with missing data filled")+xlab("Steps")+ylab("Interval")
...
g<-ggplot(Activity2, aes(x=interval, y=steps, color=TypeOfweek))

# Setting shape for the plot

g<-g+geom_line()+ facet_wrap(~TypeOfweek, ncol=1,nrow=2)

# Setting names for the plot

g<-g+labs(title="Average steps for Weekdays & Weekends", x="Interval", y="Steps")
